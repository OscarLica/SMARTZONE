<%@ Page Title="Productos" Language="C#" MasterPageFile="~/Principal.Master" AutoEventWireup="true" CodeBehind="MainProductosAlmacen.aspx.cs" Inherits="Interface.MainProductosAlmacen" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        const Onload = async () => {
            ...
            const url = "MainProductosAlmacen.aspx/CargarProductosAlmacen";
            ...
            response.d.forEach((productos) => {
                ...
                //let btnView4 = document.createElement("input");
                //btnView4.value = "Devolución";
                //btnView4.type = "button";
                //btnView4.onclick = () => {
                //    ModalFunction('ModalVer');
                //    document.getElementById('txtImei').value = productos.IMEI;
                //    document.getElementById('txtGama').value = productos.GAMA;
                ...
                //    document.getElementById('txtProducto').value = productos.NOMREPRODUCTO;
                //}
                tdActions.append(btnView, btnView2, btnView3/*, btnView4*/);
                //apends
        ...
        const BuscarProducto = async () => {
            ...
            const IdProducto = params.get("IdProducto");
            console.log(IdProducto);
            const url = "MainProductosAlmacen.aspx/BuscarProductoxNombre";
            let response = await fetch(
                url, {
                    method: "POST",
                    headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' },
        ...
        const DarBaja = async () => {
            ...
            document.getElementById('Cuerpo').innerHTML = ''; //Aqui limpiamos el la grid
            ModalFunction('ModalBaja');
            const url = "MainProductosAlmacen.aspx/DarBaja";
            let response = await fetch(
                url, {
                    method: "POST",
                    headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' },
                    body: JSON.stringify({ iddetalle: txtIdBaja.value, idmaestro: txtId2.value, motivo: document.getElementById("MotivoBaja").value })
                });
            response = await response.json();
            console.log(response);
            let Frag = document.createDocumentFragment();
    ...
    <div class="GroupDiv" id="ProductosContainer">
        ...
        <div class="Content">
            ...
            <div class="ModalActions" style="text-align: center">
                <input type="button" class="btnCancel" onclick="window.location = 'MainProductos.aspx'" value="Regresar" />
            </div>
        </div>
    </div>
    <%--Modficar--%>
    ...
    <div class="Modal" id="ModalBaja">
        <div class="ModalContent">
            ...
            <div class="ModalActions" style="text-align: center">
                ...
                <textarea class="txtForm" id="MotivoBaja" placeholder="Ingrese el motivo de baja"></textarea>
            </div>
